...
* @LastEditors: Roy
* @LastEditTime: 2020-09-27 18:11:33
* @Deprecated: 否
* @FilePath: /etf_flutter/lib/pages/contract/widgets/market/clinch_deal.dart
*/
import 'dart:async';
...
class _ClinchDealState extends State<ClinchDeal> {
  ...
  _listView(ContractOrderProvider model) {
    return model.tradeHistoryModelList.asMap().keys.map((index) {
      CKlineTradeModel tradeHistory = model.tradeHistoryModelList[index];
      return Container(
        height: height(80),
        ...
        child: Row(
          ...
          children: <Widget>[
            ...
            Expanded(
              child: Text(
                tradeHistory.direction == '-' ? '-' : tradeHistory.direction == 'buy' ? '${Tr.of(context).tradrBuy}' : '${Tr.of(context).tradrSell}',
                style: TextStyle(color: tradeHistory.direction == 'buy' ? kRedColor : kGreenColor, fontSize: sp(24)),
                textAlign: TextAlign.center,
              ),
            ),
            Expanded(
