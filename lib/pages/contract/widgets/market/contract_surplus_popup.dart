...
class CheckSurplusView extends StatefulWidget {
  CheckSurplusView({Key key, this.onSelect, this.model}) : super(key: key);
  final Function onSelect;
  final CurrentHouseModel model;
  @override
  _CheckSurplusViewState createState() => _CheckSurplusViewState();
}

class _CheckSurplusViewState extends State<CheckSurplusView> {
  String numHand = '25';
  String close = '--';
  num earnings = 0;
  Timer timer;
...
// 开多时:止盈 = (填入价格-开仓均价) * 手数 * hand_num
// 开空时:止盈 = （开仓均价 - 填入价格）* 手数 * hand_num
// 填入价格判断：
// 开多时：止盈-填入价格必须大于开仓均价
// 开空时：止盈-填入价格必须小于开仓均价
    ...
    priceCtr.addListener(() {
      ...
      if (widget.model.markType == 1) {
        if (num.parse(priceCtr.text) < num.parse(widget.model.openPrice)) {
          ...
          setState(() {
            priceCtr.text = widget.model.openPrice;
            price = priceCtr.text;
            getEarnings();
          });
        } else {
          ...
          child: Column(
            ...
            children: <Widget>[
              Container(
                child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: <Widget>[
                  RichText(
                    ...
                    children: [TextSpan(text: '${Tr.of(context).contractStoProfit}', style: TextStyle(fontSize: sp(30), color: kGreenColor))],
                  )),
                  InkWell(
              ...
              Container(
                ...
                child: Row(
                  children: <Widget>[
                    Text('${Tr.of(context).contractMaximumProfit}',
                        style: TextStyle(
              ...
              Container(
                ...
                child: Row(
                  children: <Widget>[
                    Text('${Tr.of(context).contractMaximumProfitNumber}',
                        style: TextStyle(
                          color: Color(0xff707070),
                          fontSize: sp(28),
                          fontWeight: FontWeight.bold,
                            ...
                            Expanded(
                              child: FocusWidget(
                                ...
                                child: InputWidget(
                                  ...
                                  focusNode: numberFocus,
                                  hintText: '${Tr.of(context).contractMaximumProfitNumber}${widget.model.totalHand}(${Tr.of(context).contractSingleLargest}1000)',
                                  maxHeight: 60,
                                  border:
                                      UnderlineInputBorder(borderSide: BorderSide(width: 0, color: Colors.transparent)),
              ...
              PercentageButton(
                ...
                onSelect: (e) {
                  ...
                  print('${Tr.of(context).contractMaximumProfitNumber}：：：：$handNum');
                  setState(() {
  ...
  contractWin() async {
    ...
    try {
      await ContractServer.contractWin(widget.model.positionId.toString(), price, number, '1');
      Toast.close();
