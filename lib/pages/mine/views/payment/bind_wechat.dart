...
class BindWeChatPage extends StatefulWidget {
  @override
  _BindWeChatPageState createState() => _BindWeChatPageState();
}
enum LoadType { success, error, waiting, loading }
class _BindWeChatPageState extends State<BindWeChatPage> {
                ...
                FocusWidget(
                  ...
                  child: InputWidget(
                    ...
                    prefixIcon: Container(
                      child: Text('${Tr
                          .of(context)
                          .c2cWeChatNickname}：', style: TextStyle(color: Colors.black)),
                    ),
                  ),
                ),
                FocusWidget(
        ...
        try {
          ...
          if (response != null) {
            setState(() {
              ...
              _frontImage = response['data']['fileUrl'].toString();
  ...
  void _confirm() async {
    ...
    if (_nameCtr.text.isEmpty) {
      ...
      try {
        await C2cServer.bindWechat(_nameCtr.text?? '', _NumberCtr.text ?? '',_frontImage, _codeCtr.text, _ecodeCtr.text, _gcodeCtr.text);
