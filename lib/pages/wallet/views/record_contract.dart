...
*/
import 'package:HBit/common/dropdown.dart/index.dart';
...
import 'package:HBit/pages/wallet/widgets/RecordContractItemWidget.dart';
...
class RecordContractPage extends PageList {
  RecordContractPage({Key key}) : super(key: key);

  @override
  _RecordContractPageState createState() => _RecordContractPageState();
}

class _RecordContractPageState extends PageListState<RecordContract, RecordContractPage> {
  Map params = {"coin_id": '', "type": 0, "time": 0};
  String symbol = '请选择';
  GlobalKey _globalKey = GlobalKey();
  @override
  Widget build(BuildContext context) {
    symbol = Tr.of(context).assetChoose;
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(
          '${Tr.of(context).assetContractrecord}',
  ...
  ///加载数据
  @override
  loadPage({int page, Function(List<RecordContract>) onSuccess, Function onErr}) async {
    try {
      List<RecordContract> list = await ContractWalletServer.contractRecord({
        "symbol": '',
        "page": page,
        "per_page": 10,
      });
  ...
  @override
  buildItem(RecordContract model) {
    return Container(
      padding: EdgeInsets.symmetric(horizontal: width(24)),
      child: RecordContractItemWidget(model: model),
    );
  }
