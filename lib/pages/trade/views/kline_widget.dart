    ...
    return Column(
      children: <Widget>[
        ...
        Row(
          ...
          children: <Widget>[
            _periodBtn('1${Tr.of(context).klineMinute}', '${60 * 1000}', false),
            _periodBtn('5${Tr.of(context).klineMinute}', '${5 * 60 * 1000}', false),
            _periodBtn('1${Tr.of(context).klineDay}', '${24 * 60 * 60 * 1000}', false),
            FSuper(
    ...
    if (selectMore) {
      return Row(
        ...
        children: <Widget>[
          ...
          _periodBtn('15${Tr.of(context).klineMinute}', '${15 * 60 * 1000}', true),
          _periodBtn('30${Tr.of(context).klineMinute}', '${30 * 60 * 1000}', true),
          _periodBtn('1${Tr.of(context).klineHour}', '${60 * 60 * 1000}', true),
//          _periodBtn('4小时', '${4*60*60*1000}', true),
  ...
  _getData() async {
    ...
    kLinelist = marketData.reversed
        .map((item) => KLineEntity(
            ...
            vol:double.parse(item.volume.toString()) ,
            id: item.time))
        .toList();
  ...
  Future<String> getIPAddress(String time) async {
    var url =
        GlobalConfig.apiHost1 + "/api/spot/market/kline?symbol=${widget.coinId}&range=${period ?? 60000}&limit=200";
    print(url);
    String result;
  ...
  Future<String> getLastKline() async {
    var url = GlobalConfig.apiHost1 + "/api/spot/market/last_kline?symbol=${widget.coinId}&range=${period ?? 60000}";
    String result;
    var response = await http.get(url);
    if (response.statusCode == HttpStatus.ok) {
      result = response.body;
