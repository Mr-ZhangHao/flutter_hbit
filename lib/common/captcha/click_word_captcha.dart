...
import 'package:HBit/common/captcha/HttpManager.dart';
import 'package:HBit/common/captcha/object_utils.dart';
import 'package:HBit/common/captcha/widget_util.dart';
import 'package:HBit/utils/encrypt_util.dart';
...
class _ClickWordCaptchaState extends State<ClickWordCaptcha> {
  ...
  _loadCaptcha() async {
    ...
    _changeResultState();
    var res = await HttpManager.requestData('/captcha-api/captcha/get', {"captchaType": "clickWord"}, {});
    if (res['repCode'] != '0000' || res['repData'] == null) {
      ...
      var baseR =
          await WidgetUtil.getImageWH(image: Image.memory(Base64Decoder().convert(_clickWordCaptchaModel.imgStr)));
      baseSize = baseR.size;
...
//    Map _map = json.decode(dcrypt);
    var res = await HttpManager.requestData('/captcha-api/captcha/check',
        {"pointJson": cryptedStr, "captchaType": "clickWord", "token": _clickWordCaptchaModel.token}, {});
    if (res['repCode'] != '0000' || res['repData'] == null) {
      _checkFail();
      return;
    }
    ...
    if (repData["result"] != null && repData["result"] == true) {
      ...
      if (!ObjectUtils.isEmpty(_clickWordCaptchaModel.secretKey)) {
        ...
        captchaVerification =
            EncryptUtil.aesEncode(key: _clickWordCaptchaModel.secretKey, content: captchaVerification);
      }
    ...
    return Scaffold(
      ...
      body: Center(
        child: Container(
          ...
          color: Colors.white,
          child: Column(
            children: <Widget>[_topConttainer(), _captchaContainer(), _bottomContainer()],
          ),
        ),
      ),
    );
    ...
    for (int i = 0; i < _tapOffsetList.length; i++) {
      ...
      _widgetList.add(Positioned(
          ...
          child: Container(
            ...
            height: _widgetW,
            decoration:
                BoxDecoration(color: Color(0xCC43A047), borderRadius: BorderRadius.all(Radius.circular(_widgetW))),
            child: Text("${i + 1}", style: TextStyle(color: Colors.white, fontSize: 15)),
          )));
    }
  ...
  _bottomContainer() {
    return Container(
      ...
      width: baseSize.width,
      decoration:
          BoxDecoration(borderRadius: BorderRadius.all(Radius.circular(4)), border: Border.all(color: borderColor)),
...
class ClickWordCaptchaModel {
  ...
  ClickWordCaptchaModel(
      {this.imgStr = "", this.token = "", this.secretKey = "", this.wordList = const [], this.wordStr = ""});
